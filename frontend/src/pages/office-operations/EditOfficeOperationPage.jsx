...
export const EditOfficeOperationPage = () => {
  ...
  const { data: operationData, isLoading: operationLoading } = useQuery({
    queryKey: ['officeOperation', id],
    queryFn: () => officeOperationsAPI.getById(id),
    enabled: !!id,
  });

  const operation = operationData?.data?.officeOperation;
  ...
  const updateMutation = useMutation({
    mutationFn: (data) => officeOperationsAPI.update(id, data),
    onSuccess: () => {
      queryClient.invalidateQueries(['officeOperations']);
      queryClient.invalidateQueries(['officeOperation', id]);
    ...
    onError: (error) => {
      const errorMessage = error.response?.data?.message || error.message || 'حدث خطأ أثناء تحديث العملية';
  ...
  const onSubmit = (data) => {
    ...
    console.log('Updating operation:', formattedData);
  ...
  if (!operation) {
    return (
      <>
        ...
        <div className="flex items-center justify-center min-h-screen">
          <div className="text-center">
            <p className="text-gray-600 text-lg">العملية غير موجودة</p>
            <Button onClick={() => navigate('/office-operations')} className="mt-4">
    ...
    <>
      ...
      <div>
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            <h1 className="text-3xl font-bold text-gray-800">تعديل عملية المكتب</h1>
            <p className="text-gray-600 mt-1">تحديث معلومات عملية المكتب</p>
          </div>
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
          <div className="space-y-6">
            ...
            <div className="flex gap-3 justify-end">
              ...
              >
                {updateMutation.isLoading ? 'جاري التحديث...' : 'تحديث العملية'}
              </Button>
            </div>
          </div>
        </form>
