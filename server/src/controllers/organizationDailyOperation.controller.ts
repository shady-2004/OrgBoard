...
const createOrgDailyOperation = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
    ...
    const organization = await Organization.findById(orgDailyOperationData.organization);
    if (!organization) {
        return next(new AppError("Organization not found", 404));
    }

    const operation = await DailyOrganizationOperation.create(orgDailyOperationData);
    await operation.populate("organization", "ownerName");
...
const updateOrgDailyOperation = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
    ...
    const updateDailyOperationSchemaZod = dailyOrganizationOperationSchemaZod.partial();
    const result = updateDailyOperationSchemaZod.safeParse(body);
    if (!result.success) {
